...
const Parallax = (): JSX.Element => {
 const parallax = (event: MouseEvent) => {
  document.getElementById("wrap")?.childNodes.forEach((shift: ChildNode) => {
   const position = (shift as HTMLSpanElement).getAttribute("id")
   ...
   ;(shift as HTMLSpanElement).style.transform = `translateX(${x}px) translateY(${y}px)`
  })
 }
 ...
 const parallaxTouch = (event: TouchEvent) => {
  document.getElementById("wrap")?.childNodes.forEach((shift: ChildNode) => {
   ...
   ;(shift as HTMLSpanElement).style.transform = `translateX(${x}px)`
  })
 }
